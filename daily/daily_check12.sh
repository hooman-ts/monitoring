...
echo -e "\e[39m \e[38;5;45m Daily Check v1.1 Buid 84 for Oracle Databases 12.1 and later \e[39m"
...
# CDB or NON-CDB
echo -e "\e[39m \e[38;5;154m ***** CDB or NON-CDB ***** \e[39m"
printf "\n"

sqlplus -s / as sysdba << EOF
SET LINE 120 SERVEROUTPUT ON FEEDBACK OFF
DECLARE
    CURSOR c1 IS SELECT PDB_NAME FROM DBA_PDBS WHERE PDB_NAME <> 'PDB\$SEED';
    typ   VARCHAR2 (20);
    pdb   VARCHAR2 (50);
BEGIN
    SELECT CDB INTO typ FROM V\$DATABASE;
    IF typ = 'YES'
    THEN
        BEGIN
            DBMS_OUTPUT.PUT_LINE (
                'Your Database is CDB and have the following PDBs:');            
            OPEN c1;
            LOOP
                FETCH c1 INTO pdb;
            ...
            END LOOP;
            DBMS_OUTPUT.PUT_LINE ('You should run ./daily_check12_PDB.sh for each PDB');
        END;
    ELSE
        DBMS_OUTPUT.PUT_LINE ('Your Database is NON-CDB ');
    END IF;
END;
/
EXIT
...
# Cluster or Single
echo -e "\e[39m \e[38;5;154m ***** CLUSTER or SINGLE ***** \e[39m"
printf "\n"
...
DECLARE
    ...
    s     VARCHAR2 (50);
BEGIN
    SELECT VALUE into typ FROM V\$PARAMETER WHERE NAME = 'cluster_database';
    IF typ = 'TRUE' THEN
       begin
          select VALUE into s FROM V\$PARAMETER 
          WHERE NAME = 'cluster_database_instances';
          s := 'Database Type is RAC and have '||s||' Instances';
       end; 
    ELSE
        s := 'The Database is Single Instance ';
    END IF;
...
select sequence#, to_char(completion_time, 'yyyy/mm/dd hh24:mi:ss') 
from v\$archived_log where recid = (select min(recid) from v\$archived_log where DELETED <> 'YES' and STANDBY_DEST = 'NO');
EXIT
EOF
...
printf "\n\n"
dmesg -TL|grep -i -C 3 'fail\|error'
